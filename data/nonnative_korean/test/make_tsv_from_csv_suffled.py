...
with open('0.csv', 'r', encoding='utf-8') as f: lines = f.readlines()
...
for line in lines:
	...
	if num_frame <= 272000:
		...
		file_index = 'test_0_' + str(f_idx).zfill(6)
		f_idx += 1

		category = line.split(',')[-1][0]
		new_line = file_index + ',' + path + ',' + category + ',' + script + ',' + 'test' + '\n'
		
		new_lines.append(new_line)

with open('./less_17/0_test.csv', 'w') as f: f.writelines(new_lines)
